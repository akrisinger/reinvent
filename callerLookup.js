// This file uses the ANI phone number
    ...
    docClient.query(paramsQuery, function (err, dbResults) {
        ...
        //if no error occured, proceed to process the resutls that came back from DynamoDB
            ...
            if (dbResults.Items.length === 1) {
                ...
                var customeroutageStatus = dbResults.Items[0].outageStatus;
                var customeroutageETR = dbResults.Items[0].outageETR;
                var customeroutageCount = dbResults.Items[0].outageCount;
                var customerstreetNum = dbResults.Items[0].streetNum;
                var customeradressStreet = dbResults.Items[0].addressStreet;
                var customeraddressCity = dbResults.Items[0].addressCity;
                ...
                docClient.update(paramsUpdate, function (err, data) {
                    if (err) console.log("Unable to update item. Joe Error 1: ", JSON.stringify(err, null, 2));
 
                    else console.log("Updated item succeeded: ", JSON.stringify(data, null, 2));
 
                });
 
                callback(null, buildResponseNumberFound(customerFirstName, customerLastName, lastCalledDate, lastCalledTime, customerFirstCall, customeroutageStatus, customeroutageETR, customeroutageCount, customerstreetNum, customeradressStreet, customeraddressCity));
            } else {
                customerFirstCall = true;
                docClient.update(paramsUpdate, function (err, data) {
                    if (err) console.log("Unable to update item. Joe Error 2: ", JSON.stringify(err, null, 2));
...
function buildResponseNumberFound(customerFirstName, customerLastName, lastCalledDate, lastCalledTime, customerFirstCall, customeroutageStatus, customeroutageETR, customeroutageCount, customerstreetNum, customeradressStreet, customeraddressCity) {
    var results = {
        ...
        outageStatus: customeroutageStatus,
        outageETR: customeroutageETR,
        outageCount: customeroutageCount,
        streetNum: customerstreetNum,
        addressCity: customeraddressCity,
        addressStreet: customeradressStreet,
